...
async function stopMediaTracks() {
  ...
  const featureUseAudioOnly = [t('audioVolume.title'), t('audioMixer.title'), t('audioPlayerBgm.title'), t('audioPlayerPcm.title')];
  let stream;
  try {
...
function fillParamsFromUrl() {
  ...
  if (urlUserSig && urlSdkAppId && urlUserId && urlRoomId) {
    ...
    const inviteSectionEl = document.getElementById('invite-section-el');
    if (inviteSectionEl) {
...
function changeButtonDisplay() {
  document.querySelectorAll('.mute-btn, .publish-btn').forEach((button) => {
...
function switchButtonStatusOnAudioChange(toStart) {
  ...
  buttonIdOnChange.forEach((id) => {
    document.getElementById(id).disabled = !toStart;
  });
...
function switchElementStatusOnRoomStatus(toEnter) {
  ...
  elementIdOnChange.forEach((id) => {
    document.getElementById(id).disabled = !toEnter;
  });
...
/**
 * Initialize i18n for a demo page with optional dynamic content callback.
 * This eliminates the repetitive applyI18n + lang-changed listener boilerplate.
 * @param {Function} [updateDynamicContent] - Callback to re-apply dynamic texts on language change
 */
function initPageI18n(updateDynamicContent) {
  applyI18n();
  if (updateDynamicContent) updateDynamicContent();
...
/**
 * Auto-detect iframe mode and add 'in-iframe' class to body.
 * Call this early in each demo's script.js if needed.
 */
