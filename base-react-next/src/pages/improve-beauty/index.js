...
export default function BasicRtc(props) {
  ...
  const changeBeauty = (event, number, type) => {
    ...
    <div className={clsx(styles['content-container'], isMobile && styles['mobile-device'])}>
      ...
      <div className={clsx(styles['control-container'], isMobile && styles['mobile-device'])}>
        <div className={clsx(styles['body-container'], isMobile && styles['mobile-device'])}>
          <Accordion className={styles['accordion-container']} defaultExpanded={true}>
            ...
            <AccordionDetails className={styles['accordion-details-container']}>
              ...
              <div className={styles['beauty-container']}>
                <Typography className={styles['beauty-label']} gutterBottom>{a18n('美颜')}</Typography>
                <Slider valueLabelDisplay="auto" aria-label="slider" defaultValue={50} onChange={(event, value) => changeBeauty(event, value, 'beauty')} />
              </div>
              <div className={styles['beauty-container']}>
                <Typography className={styles['beauty-label']} gutterBottom>{a18n('高光')}</Typography>
                <Slider valueLabelDisplay="auto" aria-label="slider" defaultValue={50} onChange={(event, value) => changeBeauty(event, value, 'brightness')} />
              </div>
              <div className={styles['beauty-container']}>
                <Typography className={styles['beauty-label']} gutterBottom>{a18n('红润')}</Typography>
                <Slider valueLabelDisplay="auto" aria-label="slider" defaultValue={50} onChange={(event, value) => changeBeauty(event, value, 'ruddy')} />
              </div>
            </AccordionDetails>
          </Accordion>
          {/* 用户列表 */}
          ...
          {/* 相关资源 */}
          <RelatedResources language={language} resources={[
            { name: a18n('开启美颜教程'),
              link: 'https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-28-advanced-beauty.html',
              enLink: 'https://web.sdk.qcloud.com/trtc/webrtc/doc/en/tutorial-28-advanced-beauty.html',
            },
          ]}></RelatedResources>
        </div>
      </div>
...
export const getStaticProps = () => {
  const result = getNavConfig('improve-beauty');
  return {
    props: {
      ...result,
    },
